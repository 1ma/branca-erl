...
-import(enacl, [aead_xchacha20poly1305_ietf_decrypt/4, aead_xchacha20poly1305_ietf_encrypt/4, randombytes/1]).
...
encode(PlainText, Secret, Timestamp)
  ...
  Payload = aead_xchacha20poly1305_ietf_encrypt(PlainText, Header, Nonce, Secret),
...
decode(CipherText, Secret)
  ...
  try
    ...
    enacl_result_adapter(aead_xchacha20poly1305_ietf_decrypt(Payload, Header, Nonce, Secret))
  catch
...
decompose_token(Token) ->
  <<Header:29/bytes, Payload/binary>> = decode(Token),
  <<?VERSION_BYTE:8, Timestamp:32/integer, Nonce:?NONCE_LENGTH/bytes>> = Header,
